...
ENVDIR="$(mktemp -d)"
# BIN_DIR normally lives outside it; keep pointing at the real tools.
BIN_DIR="$REPO_ROOT/.bin"
export ENVDIR BIN_DIR
echo 'RELEASE_NAME=kcm-1-11-0' > "$ENVDIR/release.env"
cat > "$ENVDIR/kcm-values.rendered.yaml" <<'EOF'
image:
