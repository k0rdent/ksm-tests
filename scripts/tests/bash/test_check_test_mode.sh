...
TEST_MODE=adopted bash "$SCRIPTS_DIR/check_test_mode.sh" >/dev/null 2>&1
assert_eq "'adopted' is accepted (exit 0)" 0 "$?"
...
assert_contains "lists allowed values" "$out" "adopted"
