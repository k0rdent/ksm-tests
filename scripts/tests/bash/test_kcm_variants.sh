...
var_of() { # var_of KCM VAR
    KCM="$1" bash -c "
        unset KCM_MODE KCM_REF KCM_VERSION
        source '$SCRIPTS_DIR/lib/common.sh'
        echo \"\${$2}\""
}

# ── Resolution ───────────────────────────────────────────────────────────────
assert_eq "src-main builds from source" "source" "$(var_of src-main KCM_MODE)"
...
got="$(KCM=rel-1-11-0 KCM_VERSION=1.9.0 bash -c \
    "unset KCM_MODE KCM_REF; source '$SCRIPTS_DIR/lib/common.sh'; echo \$KCM_VERSION")"
