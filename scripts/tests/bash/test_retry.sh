...
out=$(MAX_RETRIES=3 SLEEP=0 bash "$SCRIPTS_DIR/utils/retry.sh" true 2>&1)
assert_eq "success -> exit 0" 0 "$?"
assert_contains "reports success" "$out" "Command succeeded"

out=$(MAX_RETRIES=3 SLEEP=0 bash "$SCRIPTS_DIR/utils/retry.sh" false 2>&1)
...
out=$(MAX_RETRIES=5 SLEEP=0 bash "$SCRIPTS_DIR/utils/retry.sh" bash -c "$body" 2>&1)
